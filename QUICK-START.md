...
To persist GitHub push auth once (no new PAT each push):
```bash
export GITHUB_USER=Samwelabuyeka
export GITHUB_TOKEN=YOUR_EXISTING_PAT
./scripts/setup-git-push-auth.sh
```

Persistent data generated by first-run setup:
- `persistent-data/market/*` (10-year OHLCV jsonl by symbol)
- `persistent-data/ai/training-corpus.jsonl` (AI training corpus)
- `persistent-data/ai/manual-model.json` (manually trained local fallback model)

Manual refresh commands (without reinstalling):
```bash
